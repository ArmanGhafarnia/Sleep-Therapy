...
def evaluate_conversation_stay_on_track(conversation):
    ...
    def evaluate_message(patient_message, therapist_message):
        ...
        patient_response = openai.ChatCompletion.create(
            ...
            messages=[
                ...
                {"role": "user",
                 "content": f"The patient's message is: '{patient_message}'. Is this message related to sleep therapy? Answer 'yes' or 'no'."}
            ]
        )
        ...
        if "no" in patient_evaluation:
            ...
            redirection_response = openai.ChatCompletion.create(
                ...
                messages=[
                    {"role": "system",
                     "content": "You are an evaluator that determines if a therapist's response successfully redirects the conversation back to sleep therapy."},
                    {"role": "user",
                     "content": f"The patient's off-topic message was: '{patient_message}'. The therapist's response is: '{therapist_message}'. Does this response bring the conversation back to sleep therapy? Answer 'yes' or 'no'."}
                ]
            )
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_evaluation = {
            executor.submit(evaluate_message, patient_message, therapist_message): (patient_message, therapist_message)
...
conversation = [
    ...
    ("Maybe. By the way, did you hear about the new movie that just came out? It’s supposed to be amazing.",
     "I haven't heard about it. Anyway, let's try to focus on your sleep for now."),
    ...
    ("Actually, did you see the football game last night? It was crazy!",
     "Yeah, it was really intense! Anyway, what else do you do before bed?"),
    # Therapist fails to properly redirect here
    ("I also think about buying new gadgets. I'm really into tech stuff.",
     "Oh, that's interesting! What kind of gadgets are you looking at?"),  # Therapist completely fails to redirect here
    ...
    ("Alright. By the way, I’m thinking of getting a new car. What do you think about electric cars?",
     "Electric cars are interesting! Which one are you planning to buy?"),
    # Therapist completely fails to redirect here
    ...
    ("Oh, I almost forgot, I wanted to ask if you know a good restaurant in town.",
     "I might have some suggestions. By the way, what else helps you unwind before bed?"),
    # Therapist fails to properly redirect here
